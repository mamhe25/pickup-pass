...
function configureAccountLink(link, role) {
  const label = link.querySelector('.pp-navlink__label');
  const ownerProfile = role === 'master_admin';

  link.href = ownerProfile ? './profile.html' : '../account.html';
  link.setAttribute('aria-label', ownerProfile ? 'My profile' : 'Account security');
  if (label) label.textContent = ownerProfile ? 'My profile' : 'Account security';

  if (ownerProfile && location.pathname.endsWith('/master-admin/profile.html')) {
...
export function mountAccountLink(mount) {
  ...
  onAuthStateChanged(auth, async user => {
    ...
    try {
      ...
      configureAccountLink(link, token.claims.role || '');
    } catch (_) {
      configureAccountLink(link, '');
    }

    authedFetch('/session/me').catch(() => {});
  });
}
