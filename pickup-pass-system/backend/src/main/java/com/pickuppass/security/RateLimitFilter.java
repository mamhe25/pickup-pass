...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    private Policy policyFor(HttpServletRequest request) {
        ...
        if ("POST".equals(method) && path.equals("/api/public/demo-requests")) return new Policy("demo-request", 5, 3600);
        if ("POST".equals(method) && path.startsWith("/api/public/demo-requests/") && path.endsWith("/verify")) return new Policy("demo-verify", 10, 900);
        if ("POST".equals(method) && path.startsWith("/api/public/demo-requests/") && path.endsWith("/resend")) return new Policy("demo-resend", 5, 3600);
